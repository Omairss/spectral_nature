...
echo "[3/10] Creating PostgreSQL flexible server"
...
echo "[4/10] Creating Log Analytics + Container Apps environment"
...
echo "[5/10] Creating ACR and building pipeline image"
az acr create \
  --resource-group "$RESOURCE_GROUP" \
  --name "$ACR_NAME" \
  --sku Basic \
...
REGISTRY_USER="$(az acr credential show -n "$ACR_NAME" -g "$RESOURCE_GROUP" --query username -o tsv)"
REGISTRY_PASS="$(az acr credential show -n "$ACR_NAME" -g "$RESOURCE_GROUP" --query passwords[0].value -o tsv)"

IMAGE="${REGISTRY_SERVER}/${CONTAINER_IMAGE_TAG}"

echo "[6/10] Creating user-assigned managed identity"
...
echo "[7/10] Assigning RBAC for storage access"
...
echo "[8/10] Creating scheduled jobs"
...
echo "[9/10] Capturing deployment outputs"
