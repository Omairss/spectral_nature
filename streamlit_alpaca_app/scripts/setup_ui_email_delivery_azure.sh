...
EMAIL_OUTPUTS_FILE="${EMAIL_OUTPUTS_FILE:-infra/.generated/email_delivery.local.env}"
TARGET="${TARGET:-dev}"
TEST_TO="${TEST_TO:-}"
...
load_deployment_context() {
  ...
  KEYVAULT_NAME="${KEYVAULT_NAME:-${AZURE_KEY_VAULT_NAME:-${KEY_VAULT_NAME:-}}}"
...
write_outputs_file() {
  mkdir -p "$(dirname "$EMAIL_OUTPUTS_FILE")"
