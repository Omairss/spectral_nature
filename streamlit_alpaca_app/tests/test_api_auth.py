...
def test_agent_key_scopes_are_normalized(monkeypatch):
    ...
    created = api_auth.create_agent_api_key(
        name="agent-one",
        scopes=["not-allowed", api_auth.SCOPE_MCP_INVOKE, api_auth.SCOPE_QUERY_EXECUTE],
    )
    assert created["api_key"].startswith("snak_")
    assert created["key"]["scopes"] == [api_auth.SCOPE_MCP_INVOKE, api_auth.SCOPE_QUERY_EXECUTE]
