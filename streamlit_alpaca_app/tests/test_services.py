...
def test_scan_momentum_profiles_returns_momentum_and_acceleration_scores():
    ...
    assert {"momentum_score", "momentum_roc_score", "roc_1w_to_1m", "roc_1m_to_3m"} <= set(out.columns)
...
def test_build_company_description_uses_asset_fundamentals_and_signal_summary():
    fundamentals = load_quarterly_fundamentals("A")
    description = build_company_description(
        "A",
        {"name": "Agilent Technologies", "exchange": "NYSE", "status": "active", "class": "us_equity"},
        fundamentals,
        {"regime": "Trend continuation", "pullback_from_ath_pct": -6.2, "dist_to_resistance_pct": 3.4},
    )

    assert "Agilent Technologies (A)" in description
    assert "Latest quarterly results" in description
...
def test_build_fred_figure_can_overlay_relative_percent_change():
    frame = pd.DataFrame(
        {
    ...
    fig = build_fred_figure(
        ...
        frame,
        show_relative_pct=True,
    )

    assert len(fig.data) == 2
    assert fig.data[1].name == "% change vs first obs"
