...
- For layout cleanup in this app, `streamlit.testing.v1.AppTest` is fast enough to verify block order and visible text without needing a browser session.
...
- When an agentic summary depends on live search, require real evidence and fall back cleanly instead of letting the summary invent a market-wide story.
...
## 40. Recurring mistakes cluster into structural patterns that need systemic fixes

- 40 mistakes over 10 days grouped into 6 root patterns: deploy safety, end-to-end verification gaps, hardcoded templates, secrets drift, stale data, and doc drift.
- One-off fixes for individual mistakes are not enough. Systemic guards (deploy guard scripts, pre-commit hooks, mandatory dataset validation, startup readiness checks) prevent entire categories at once.
- A deploy guard that classifies changed files by container (`scripts/which_deploy.sh`) prevents the most disruptive category: deploying the wrong container.
- Mandatory dataset validation in pipeline jobs prevents the second most disruptive category: green jobs that produce stale or empty output.
- Startup readiness logging (`check_llm_readiness()`) prevents the third: assuming a capability is live because the code path exists.
