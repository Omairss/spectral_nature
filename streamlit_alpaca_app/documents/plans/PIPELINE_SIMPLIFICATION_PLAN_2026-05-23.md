...
| Boundary test scans the wrong symbol | `tests/test_zopedia_native_llm_boundary.py` checks for `load_zopedia_llm_client`, while the active product goal is now the v0.6 AQL/Zopedia gateway. | A test that scans only loader names gives false confidence and lets orphan Zopedia/AQL LLM calls spread. | Replace loader-only checks with gateway enforcement. Scan direct `.generate_json(` calls and require each allowed use to be gateway internals, provider adapter internals, tests/fakes, or a temporary migration allowlist entry with owner and expiry. | The test fails before cleanup on real offenders and passes only after the owner paths are reduced to the v0.6 gateway contract. |
...
- all chat, homepage, attention, summary, maintenance, and learning research-grade model calls enter through the v0.6 AQL/Zopedia gateway and write model-call telemetry.
